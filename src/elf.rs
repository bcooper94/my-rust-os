...
//     const VALUE: Endian = Endian::Little;
...
impl Class for Elf32 {
    ...
    const VALUE: ElfClass = ElfClass::Elf32;

    fn compute_size(&self, bytes: &Self::ByteSize) -> Self::Size {
        self.endianness.get_u32(bytes)
    }
}
...
impl Class for Elf64 {
    ...
    const VALUE: ElfClass = ElfClass::Elf64;

    fn compute_size(&self, bytes: &Self::ByteSize) -> Self::Size {
        self.endianness.get_u64(bytes)
    }
}
...
struct ElfHeader {
    ...
    program_header_summary: Option<ProgramHeaderSummary>,
    section_header_summary: SectionHeaderSummary,
}

#[derive(PartialEq)]
pub struct ElfFileHandle<'a> {
    file_bytes: &'a [u8],
    header: ElfHeader,
}

impl<'a> ElfFileHandle<'a> {
    pub fn from_bytes(file_bytes: &'a [u8]) -> Result<Self, ElfParseError> {
        if !Self::is_elf_file(&file_bytes) {
    ...
    // TODO: can we statically enforce that first_bytes is len 4?
    fn is_elf_file(first_bytes: &[u8]) -> bool {
        // 0x7F followed by "ELF" in ASCII
        first_bytes.starts_with(&[0x7F, 0x45, 0x4C, 0x46])
    }

    fn parse_header(file_bytes: &'a [u8]) -> Result<ElfHeader, ElfParseError> {
        let class = ElfClass::from_byte(file_bytes[4])?;
        ...
        let program_entry_position = match class {
            ElfClass::Elf32 => endianness.get_u32(&file_bytes[24..=27].try_into().unwrap()) as u64,
            ElfClass::Elf64 => endianness.get_u64(&file_bytes[24..=31].try_into().unwrap()),
        };
        ...
        Ok(ElfHeader {
            ...
            program_header_summary: Self::parse_program_header_summary(
                file_bytes,
                &class,
                &endianness,
            ),
            section_header_summary: Self::parse_section_header_summary(
                file_bytes,
                &class,
                &endianness,
            ),
    ...
    fn parse_program_header_summary(
        ...
        class: &ElfClass,
        endianness: &Endian,
    ) -> Option<ProgramHeaderSummary> {
        match class {
            ElfClass::Elf32 => {
                let table_position =
                    endianness.get_u32(&file_bytes[28..=31].try_into().unwrap()) as u64;

                if table_position == 0 {
                    None
                } else {
                    Some(ProgramHeaderSummary {
                        table_position,
    ...
    fn parse_section_header_summary(
        file_bytes: &'a [u8],
        class: &ElfClass,
        endianness: &Endian,
    ) -> SectionHeaderSummary {
        match class {
            ElfClass::Elf32 => SectionHeaderSummary {
            ...
            ElfClass::Elf64 => SectionHeaderSummary {
...
impl<'a> Debug for ElfFileHandle<'a> {
...
mod tests {
    ...
    #[test_case]
    fn parse_main_return_0_relocatable() {
        let file_bytes = include_bytes!("elf/test_files/main_ret0.elf");
        let elf_file =
            ElfFileHandle::from_bytes(file_bytes).expect("The file should be correctly parsed");
        let expected_elf = ElfFileHandle {
            file_bytes,
            header: ElfHeader {
    ...
    #[test_case]
    fn parse_hello_world_executable() {
        let file_bytes = include_bytes!("elf/test_files/hello_world.elf");
        let elf_file =
            ElfFileHandle::from_bytes(file_bytes).expect("The file should be correctly parsed");
        let expected_elf = ElfFileHandle {
            file_bytes,
            header: ElfHeader {
                ...
                program_header_summary: Some(ProgramHeaderSummary {
                    table_position: 64,
                    entry_size: 56,
                    entry_count: 11,
                }),
